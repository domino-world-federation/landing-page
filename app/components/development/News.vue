...
<template>
  ...
  >
    ...
    <!-- Figma states this as a wrapping row of four 862px cards inside 1760,
         which is a 2×2. Written as a real grid, and 2-up only from `menu`: each
         card is a square thumbnail plus a 36px headline, and below 1400 the pair
         leaves the headline a column narrow enough to take four lines against
         the design's two. -->
    <ul class="grid list-none gap-8 menu:grid-cols-2 lg:gap-9">
      <li v-for="(article, i) in articles" :key="article.id" class="flex">
