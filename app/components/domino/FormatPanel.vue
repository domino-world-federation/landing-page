 ...
 *
 * The eyebrow reads "OVERVIEW" on both panels, which is what Figma writes. It is
 * not a heading — it labels the pair rather than the panel — so it is a plain
 * `<p>` and the format name below it is the `<h2>`.
 */
...
defineSlots<{
  /**
   * This panel's share of the artwork. Painted behind the copy and clipped by
...
const end = computed(() => props.align === "end")
...
const rootClass = computed(() =>
  cn(
    ...
    end.value && "items-end text-right",
...
<template>
  <div :class="rootClass">
    ...
    <div
      ...
      "
    >
      <MotionReveal :y="40" :class="cn('w-full', end && 'flex justify-end')">
        <div :class="cn('flex flex-col gap-4 lg:gap-9', end && 'items-end')">
          ...
          <!-- Bebas 76/72 — `--text-display-sm` exactly, the same step S10's and
               Overview's headings take. `leading-[0.95]` is Figma's 72 on a 76
               body. -->
          <h2
            :id="headingId"
      ...
      <!-- 20/32 at 60% of `#0E0E0E` — the page background used as ink, which is
           what puts the body a step behind the heading without introducing a
      ...
      <!-- A `<dl>` because each row is a term and its value — "Players: 1 vs 1"
           is a definition list said out loud, and a screen reader pairs them
           without the markup having to explain itself. The rows are
           `justify-between` with 12px of padding, 12px apart (`207:15574`). -->
      <MotionReveal :y="24" :delay="STAGGER * 2" class="w-full">
        <dl class="flex w-full flex-col gap-3">
          <div
            v-for="stat in format.stats"
            :key="stat.id"
            class="flex items-center justify-between gap-4 py-3"
          >
            <!-- Source order is label-then-value on both panels so the reading
                 order never depends on which side the panel is on; `order` flips
