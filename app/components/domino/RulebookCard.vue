 ...
 * takes the white hairline it needs to separate from the background — the chrome
 * is extrapolated (the wireframe is greyscale), the structure is not.
 *
 ...
 */
const props = defineProps<{ doc: ResourceDocument }>()
...
// "DOWNLOAD PDF ()" — an empty bracket that looks like a bug rather than like a
// size nobody supplied.
const cta = computed(() =>
  props.doc.fileSize
    ? REGULATIONS_COPY.downloadCta
        .replace("%t", props.doc.fileType)
        .replace("%s", props.doc.fileSize)
    : REGULATIONS_COPY.downloadCtaNoSize.replace("%t", props.doc.fileType),
)
</script>

<template>
  <article
    class="group relative flex h-full flex-col gap-4 rounded-[var(--radius-card)] border border-white/12 bg-black p-8 lg:p-[2.5vw]"
  >
    <!-- A 30px inline SVG sized in CSS. Drawn in `#1A1C1D` for light grounds, so
         `invert` turns it white for this one — the same trick S8's arrow uses. -->
    <img
      src="/assets/global/icon-download.svg"
      alt=""
      width="30"
      height="30"
      class="size-[30px] shrink-0 invert transition-transform duration-200 group-hover:translate-y-0.5"
    >
    ...
    <p
      ...
      class="font-sans text-[length:var(--text-eyebrow)] leading-8 text-white/80"
    >
      {{ doc.description }}
    </p>

    <!-- `mt-auto` pins the button to the foot however tall the panel grows — the
         wireframe puts it at the bottom of a fixed 365px cell, and this is that
         placement without the fixed height. The `pt-4` is `119:4590`, which
         spends 16px above the button on top of the panel's own 16px gap; the
         wrapper exists to carry it, because putting it on the button would grow
         the button rather than the space above it.
         ...
         visible. -->
    <div aria-hidden class="pointer-events-none mt-auto pt-4">
      <div
        class="font-display group-focus-within:ring-gold flex items-center justify-center rounded-[var(--radius-btn)] border border-white px-5 py-2 text-center text-[length:var(--text-display-btn)] leading-10 text-white uppercase transition-colors group-hover:bg-white/12 group-focus-within:ring-2"
      >
        {{ cta }}
      </div>
    </div>
