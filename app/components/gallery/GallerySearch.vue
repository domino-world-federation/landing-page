 ...
 *
 * The news header's field with this page's words in it; see `NewsSearch` for the
 * full reasoning. **It does not search** — there is no endpoint (B2) and nothing
 * loaded to match against — so submitting says so through `role="status"` rather
 * than appearing to look and finding nothing (D28).
 */
const query = ref("")
const notice = ref("")

function handleSubmit() {
  notice.value = GALLERY_COPY.searchUnavailable
}
</script>
...
<template>
  <div class="flex w-full flex-col gap-2 lg:max-w-[402px]">
    ...
    >
      ...
      >
        <!-- A 36px inline SVG sized in CSS. Drawn in white already, so no
             `invert`. Moved from `news/` to `global/` when the FAQ header became
             its third user, which is what its old note said would move it
             (D32/D43). -->
        <img
    ...
    <!-- Always in the DOM, empty until there is something to say: a live region
         ...
         readers, which need it present in order to watch it. -->
    <p role="status" class="font-sans text-sm leading-5 text-white/60 empty:hidden">
      {{ notice }}
    </p>
  </div>
</template>
