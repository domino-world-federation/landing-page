  ...
  >
    ...
    <MotionEntranceGroup :amount="0.4" class="absolute inset-0 z-10">
      ...
      <MotionParallaxLayer
        ...
        anchor="top"
        decorative
        class="absolute top-[43%] -left-[16%] z-20 w-[78%] lg:top-[62.5%] lg:left-[8.4%] lg:w-[49.2%]"
      >
      ...
      <!-- Height is pinned as well as width: Figma crops this one to a 585 × 636
           ...
           It straightens as it arrives. The asset is drawn 26.3° off vertical
           already and Figma adds no rotation of its own, so `0` is the raw lean
           — it starts there and pulls back to the resting angle, which reads as
           the tile righting itself rather than switching on. It stops well short
           of upright; the slant is the design. -->
      <MotionParallaxLayer
        :speed="24"
        :enter="{
          opacity: [0, 1],
        ...
        anchor="top"
        class="absolute top-[16%] left-[43%] z-30 h-[15%] w-[30%] lg:top-[16.9%] lg:left-[35.3%] lg:h-[61.2%] lg:w-[30.5%]"
      >
      ...
      <div
        aria-hidden="true"
        class="absolute inset-x-0 bottom-0 z-40 hidden h-[36%] bg-[radial-gradient(ellipse_130%_100%_at_51%_0%,transparent_47%,var(--color-bg)_100%)] lg:block"
      />
    </MotionEntranceGroup>
    ...
    <!-- `mt-auto` here and on the CTA block below splits the leftover height
         ...
         moment. -->
    <p
      class="font-sans relative z-50 mt-auto pt-28 bg-linear-to-r from-white via-[var(--color-silver-mid)] to-white bg-clip-text px-5 text-center text-base font-semibold tracking-[0.24em] text-transparent uppercase lg:absolute lg:inset-x-0 lg:top-[36%] lg:mt-0 lg:pt-0 lg:text-2xl"
    >
      {{ HERO_COPY.tagline }}
    </p>
    ...
    <h1
      class="font-display relative z-0 mt-3 bg-linear-to-r from-white via-[var(--color-silver-mid)] to-white bg-clip-text px-5 text-center text-[length:var(--text-display-lg)] leading-none text-transparent uppercase lg:absolute lg:inset-x-0 lg:top-[44%] lg:mt-0"
    >
      {{ HERO_COPY.headline }}
    </h1>

    <!-- Anchored to the bottom (`mt-auto`), so both the gap and the bottom
         padding push the CTA upwards. Figma leaves 63px between the button and
         the mission copy, and 107px below the last element.
    ...
    <div
      class="relative z-60 mt-auto flex flex-col gap-10 px-5 pt-12 pb-10 md:px-10 lg:gap-[3.33vw] lg:px-20 lg:pt-0 lg:pb-[5.6vw]"
    >
      ...
      <UiGoldCta href="#" class="mx-auto w-full max-w-70 sm:max-w-85">
