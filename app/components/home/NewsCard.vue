 ...
 */
type NewsCardSize = "tall" | "wide" | "square"
...
/**
 * The square cards are less than half the width of the others and carry the same
 * 24px title in Figma, which at that size runs to four or five lines and fills
 * the whole tile. Dropping to 18px keeps the title to the two lines the design
 * shows without changing the type scale of the big cards.
 */
...
<template>
  <article v-bind="passThrough" :class="rootClass">
    <NuxtImg
    ...
    <!-- Node `55:3216`: `rgba(0,0,0,0) → 0.7 at 50% → 1`, drawn in CSS rather
         ...
         three different heights — a baked gradient would sit at the wrong place
         in two of them.

         Figma gives this frame 331 of the card's 700px, i.e. the bottom 47%.
         Taken as a PERCENTAGE it fails on the short cards: 47% of the 418px slot
         is 196px and of a 258px square only 121px, so the wash gets steeper as
         the tile gets shorter and the title ends up sitting on picture rather
         than on shadow — measured on the `wide` card, the headline landed on a
         sunlit chair with nothing under it.

         A fixed 340px instead, which is the design's own figure at design size,
         capped at 70% so it cannot swallow a small tile whole. The scrim is then
         the same depth under every headline regardless of the slot it is in.

         `to top` with the stops reversed so the percentages read from the bottom
         edge, which is where the design measures them. -->
    <div
      aria-hidden
      class="absolute inset-x-0 bottom-0 h-[min(70%,340px)] bg-[linear-gradient(to_top,rgba(0,0,0,1)_0%,rgba(0,0,0,0.7)_50%,rgba(0,0,0,0)_100%)]"
    />

    <!-- Figma pads this 46/24/24, but the top padding only exists to clear the
         scrim's soft edge — the text is bottom-anchored, so it is the bottom and
    ...
    <div class="absolute inset-x-0 bottom-0 flex flex-col gap-2 p-4 lg:p-6">
      ...
      <!-- Clamped to two lines. Figma writes each headline short enough to fit
           ...
           next one will — an unclamped title on a 258px square grows upward
           until it fills the tile and runs off the top of its own scrim.
           `line-clamp` is the only way to bound it without measuring text, and
           it degrades to a plain overflow elsewhere. -->
      <h3
        :class="
          cn(
