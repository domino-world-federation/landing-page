 ...
 * markers are drawn rather than left to the browser, because the heading and the
 * number sit on one line at 36/48 and a list marker would not.
 */
...
<template>
  ...
  >
    <ol class="flex flex-col gap-8 lg:gap-12">
      <!-- The rule between clauses (`174:11263`), 4px of `--color-divider`. On
           the item and skipped for the first, so n clauses carry n−1 rules and
           none at the head. -->
      <li
        v-for="(section, index) in sections"
        :key="section.id"
        class="flex flex-col gap-8 border-t-4 border-[var(--color-divider)] pt-8 first:border-t-0 first:pt-0 lg:gap-12 lg:pt-12 lg:first:pt-0"
      >
        <div class="flex flex-col gap-6">
          ...
          <h2
            :id="section.id"
            class="font-sans scroll-mt-32 text-[length:var(--text-body-lg)] leading-[1.33] font-semibold text-black"
          >
