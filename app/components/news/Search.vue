...
function handleSubmit() {
  notice.value = NEWS_HEADER_COPY.searchUnavailable
}
</script>
  ...
  <div class="flex w-full flex-col gap-2 lg:max-w-[402px]">
    ...
    >
      ...
      <!-- `w-0 flex-1`, not `min-w-0`: a text input's default `size` is a
           ...
           floor alone leaves it still asking for ~187px whenever anything
           measures the box intrinsically (D29).

           Placeholder at 60% white per `176:11862`; the typed value is full
           white, or the reader cannot tell what they wrote from what they have
           not. -->
      <input
        id="news-search"
        v-model="query"
