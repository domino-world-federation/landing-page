 ...
 * 20px radius — which is what makes it a shared card rather than two that happen
 * to look alike.
 */
defineProps<{
...
<template>
  <article
    class="rounded-card flex min-w-0 grow basis-[min(100%,560px)] items-end justify-between gap-4 bg-white px-5 py-4"
  >
    <div class="flex min-w-0 flex-col gap-2">
      <!-- Inter Medium 20/28 in `#999999`. The date is optional on the type
           because most of the library carries none; a card missing one drops the
           line rather than printing an empty row. -->
      <time
    ...
    <UiDownloadPill :document="document" :label="downloadLabel" />
  </article>
</template>
