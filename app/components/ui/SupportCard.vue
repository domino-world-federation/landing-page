...
<template>
  <div class="flex flex-col gap-4 rounded-[var(--radius-glass)] bg-white p-4">
    ...
    <h2
      class="font-sans text-[length:var(--text-display-caption)] leading-[1.25] font-semibold text-black"
    >
      <!-- Rendered as blocks rather than with a `<br>`, so the words stay
           separate strings for translation. -->
      <span
        v-for="line in SUPPORT_CARD_COPY.title"
        :key="line"
        class="block"
      >{{ line }}</span>
    </h2>

    <p class="font-sans text-muted text-sm leading-[1.57] font-medium">
      {{ SUPPORT_CARD_COPY.body }}
    </p>

    <NuxtLink
      :to="SUPPORT_CARD_COPY.href"
      class="font-display flex items-center justify-center rounded-[4px] bg-[#1a1c1d] px-4 py-3 text-[length:var(--text-display-caption)] leading-[1.25] text-white uppercase transition-opacity hover:opacity-90 focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2 focus-visible:outline-none"
    >
