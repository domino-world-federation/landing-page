...
| D68 | Figur format split jadi **dua foto berbeda**, dan logika jaga-jahitan dicabut | File terbaru mengganti satu ubin bergambar globe yang membelah jahitan (`imageRef` yang sama di kedua panel) dengan **dua foto berbeda** (`272:15631` dan `272:15635`): siluet satu pemain di sisi singles, sepasang saling membelakangi di sisi doubles, keduanya opacity 0.4. Yang menarik adalah apa yang ikut hilang. Versi lama menuntut kedua panel berbagi satu `speed` parallax dan offsetnya dinyatakan sebagai persen lebar panel supaya jaraknya tetap persis satu panel di tiap lebar — kalau tidak, ubinnya sobek saat halaman di-scroll. Dengan dua gambar terpisah **tidak ada yang bisa sobek**, jadi seluruh alasan itu gugur; parallax-nya **dicabut seluruhnya**, karena Figma memakukan tepi bawah tiap gambar ke dasar panelnya dan offset parallax mengangkatnya — membuka celah di bawah kaki yang melebar seiring scroll. Sesuatu tidak bisa sekaligus berpijak dan melayang, jadi figurnya kini digantung dari `bottom-0`. Gambar barunya juga lebih baik secara argumen: ia **menyampaikan** isi section-nya (satu lawan dua) alih-alih menghias. Sekalian, cacat yang D44 tandai ikut tertutup — body panel doubles yang dulu duplikat byte-per-byte dari singles kini punya teksnya sendiri, dan teks itu persis yang sudah ada di wireframe sejak awal. Penanda `TODO(design)`-nya dicabut, dan itu argumen kecil untuk membiarkan cacat desain terlihat di kode alih-alih diam-diam ditambal: yang ditandai akhirnya dijawab | 2026-08-25 |
| D69 | Penanda peta Members jadi **data yang bisa diklik dan bernama**, posisinya **dibangkitkan dari artwork** — dan sebuah offset yang salah membuatnya meleset | Diminta pengguna: pin Jakarta dipaskan ke titiknya, titik lain bisa diklik, garis penunjuknya tinggi, dan semua tempat diberi nama. **(a) Penandanya ternyata bisa dijangkau.** D63 mengira ke-57-nya ter-bake dalam satu ekspor rata; keliru — `world-map-dots.svg` isinya 57 lingkaran dan **nol daratan**, sementara petanya ada di `decor-map-frame.svg` (satu path, 107 subpath). Koordinatnya dibaca langsung dari artwork ke `content/members/map-markers.ts` sebagai persen: **dibangkitkan, bukan ditempatkan tangan**, jadi ekspor ulang berarti bangkitkan ulang. Lapisan SVG-nya tetap dipakai apa adanya sebagai visual; yang ditambahkan hanya target klik transparan, jadi tak ada rupa penanda yang bergantung pada CSS meniru gradien dan glow. **(b) Garis penunjuk desain digeneralisasi, bukan dipendekkan.** Figma menjatuhkan garis 624px dari tag dekat puncak blok ke penandanya (`404:28201`), dan tingginya itulah gunanya — ia yang membuat titik 11px bisa ditemukan di peta selebar dua ribu piksel. Ditiru dengan menambatkan callout ke **puncak layer penanda** di kolom penanda yang terbuka, lalu membiarkan garisnya mengisi sisa jarak ke bawah; tidak ada panjang yang ditulis di mana pun, dan ia memendek sendiri untuk penanda yang tinggi. **(c) Offset yang salah, dan ini pelajaran utamanya.** Figma menaruh layer peta di x:253 y:189 dan frame-nya di x:100 y:90, keduanya relatif terhadap blok 1920×1089 — membaginya dengan **blok** memberi 13,2%/17,4%/78%. Tapi frame-nya sendiri hanya 1719×941 dan dirender selebar container, jadi angkanya harus relatif terhadap **frame**: `(253−100)/1719`, `(189−90)/941`, `1497/1719` = **8,90%/10,52%/87,09%**. Selisihnya menggeser **setiap** penanda ~4% dan cukup untuk melempar Jakarta ke Samudra Hindia. Ditemukan pengguna, bukan oleh kode — dan tidak akan pernah tertangkap `tsc` maupun `eslint`, karena keduanya angka yang sah. **(d) Nama.** Hanya "Jakarta, ID" milik desain, diidentifikasi dengan **diukur**: garis penunjuk Figma mendarat 3px dari penanda tier national di (1171, 572), dan tier-nya cocok dengan label tag-nya sendiri. 56 sisanya dibaca dengan merender tiap penanda di atas peta dengan namanya tertulis, lalu memeriksa balik negaranya — dua sempat tertukar (Kenya ↔ DR Congo) dan ketahuan justru lewat pemeriksaan itu. Karena itu namanya **negara, bukan kota**: titik di dalam Prancis tidak berkata Paris. Semuanya **placeholder** setingkat isi `mock/` lain, bukan anggota yang dikonfirmasi federasi. **Filter tier tetap tidak dihidupkan** (D63): tahu di mana penanda berada bukan berarti tahu apa dia | 2026-08-25 |
| D70 | Repo jadi **dua project terpisah** — `landing-page/` dan `backoffice/` — bukan satu app dengan route group, dan bukan workspace Bun. **Seluruh isi akar lama pindah utuh ke `landing-page/`**, `docs/` dan `deploy/` termasuk | Diminta pengguna: "beda project intinya". Backoffice adalah portal admin, bukan halaman lain dari situs publik — audiens, autentikasi, jadwal rilis, dan permukaan seranganya berbeda, jadi satu `next build` yang memuat keduanya berarti satu bug di admin bisa menjatuhkan situs publik dan tiap deploy admin memaksa situs publik ikut dibangun ulang. Pembagiannya dipilih **memindahkan akar lama apa adanya**, bukan menyisakan `docs/`, `README.md`, dan `.gitignore` di akar: dokumen dan config deploy yang ada semuanya milik situs publik, dan memindahkannya utuh membuat diff-nya rename murni — riwayat tiap file tetap terlacak, dan tidak ada path relatif di dalam dokumen yang perlu ditulis ulang. Akar tinggal dua folder project plus `CLAUDE.md` pendek yang menunjuk keduanya. Sengaja **bukan** workspace Bun: satu-satunya yang layak dipakai bersama sekarang adalah design token, dan menaruh paket bersama sebelum ada yang berbagi kode berarti memelihara lapisan kosong — kalau nanti benar-benar ada, `packages/` bisa ditambahkan tanpa membongkar pembagian ini. Isi `backoffice/` untuk sekarang kerangka folder saja, atas permintaan pengguna | 2026-08-26 |
| D71 | Seluruh **70 aset raster jadi WebP q95**, dan PNG/JPG aslinya diarsipkan ke `assets-source/` di luar `public/` — bukan diganti di tempat, bukan disandingkan sebagai fallback `<picture>` | Menutup R10, yang tertunda sejak 2026-08-22 dan sudah tumbuh jadi dua puluh lima aset di atas 1 MB. `public/assets` turun dari **77,14 MB ke 12,69 MB (−84%)**; 121 referensi di `app/` ikut ditulis ulang. Ini bukan sekadar penghematan: `nuxt.config.ts` menyetel `provider: "none"` karena box KVM produksi ada di bawah x86-64-v2 dan sharp menolak load di sana, jadi `format: ["webp"]` dan `quality: 82` di config **tidak pernah berjalan** dan seluruh 77 MB terkirim mentah. Konversi di disk adalah satu-satunya jalan yang tersisa selama IPX mati. **q95, bukan q82** yang tertulis di config: diminta pengguna dengan syarat tanpa penurunan kualitas, dan q95 memberi PSNR 48–55 dB (di atas ~45 dB mata tidak lagi memisahkan hasil dari sumbernya) sementara q82 menurunkan potret wajah close-up ke 41 dB. cwebp membuang kanal alfa pada 14 aset — diverifikasi opak penuh (SSIM kanal A 99.00), jadi lossless; 20 aset yang benar-benar transparan tetap membawa chunk `ALPH`. Aslinya **diarsipkan, tidak dibuang**, atas pilihan pengguna: sumber resolusi penuh tetap ada di working tree sebagai `assets-source/`. Folder itu **di-gitignore** — 77 MB yang tidak dipakai build maupun deploy, jadi repo tidak memikulnya dan klon baru tidak menariknya. Konsekuensinya folder itu lokal per-klon: yang membutuhkan sumbernya mengambil dari riwayat, yang menyimpan tiap aset sampai commit konversi ini (`git show <commit>^:public/assets/...`). Repo tetap tidak bertambah berat — `.webp`-nya blob baru, tapi PNG/JPG-nya sudah ada di riwayat sejak dulu. `<picture>` dengan fallback PNG ditolak — WebP didukung sejak Safari 14 (~97% global), dan dengan `provider: "none"` tidak ada negosiasi format apa pun, jadi fallback berarti memelihara dua berkas untuk tiap gambar tanpa penerima | 2026-09-03 |
| D72 | Meta SEO tiap halaman **menimpa** milik halaman, diterapkan di `app.vue` | Setiap halaman sudah punya `useSeoMeta` tulisan tangan, dan itu tetap ada — ia lantai: yang dirender sebelum API menjawab, dan yang dipakai kalau API tidak pernah ada. Tapi begitu seseorang mengetik judul di "SEO & Social", itu keputusan federasi dan harus menang. Nuxt merender head `app.vue` SEBELUM head halaman, jadi tanpa `tagPriority` halaman justru diam-diam menimpa CMS. Diterapkan sekali di `app.vue`, bukan per halaman, karena responsnya satu untuk seluruh situs; rute dinamis (`/news/[slug]`) sengaja tidak ada barisnya — metanya lahir dari record yang direndernya | 2026-09-03 |
| D73 | `useCmsSeo()` **tidak menyentuh rute yang tidak dikenal CMS** | Versi pertamanya jatuh ke `default` untuk rute tanpa baris, dan karena ia dipasang `tagPriority: 1` (supaya baris yang disunting menang), ia MENIMPA halaman yang metanya lahir dari record — judul artikel berita berubah jadi "Domino World Federation" di tab dan di setiap pratinjau tautan. Sekarang `default` adalah warisan untuk field yang dikosongkan sebuah BARIS, bukan selimut untuk rute yang federasinya tidak pernah sebutkan | 2026-09-04 |
| D74 | Halaman detail menulis tag bagikannya SENDIRI dari record-nya | Rute dinamis sengaja tidak punya baris di "SEO & Social" — satu baris akan mengklaim ribuan artikel yang semuanya berbeda. Konsekuensinya `useCmsSeo()` tidak menyentuhnya, jadi `og:*` dan `twitter:*` harus ditulis di halamannya: tanpa itu artikel yang dibagikan tampil tanpa judul, ringkasan, maupun gambar. `og:type` `article` untuk berita, dengan `article:published_time` | 2026-09-04 |
| D75 | `/sitemap.xml` DIBANGKITKAN dari API, bukan berkas statis | Rute statisnya diambil dari `/seo` — daftar halaman yang federasinya sendiri daftarkan, jadi rute yang belum punya barisnya memang belum diumumkan dan rute yang dihapus berhenti diumumkan tanpa ada yang perlu ingat. Rute dinamisnya dari `/news` dan `/tournaments`, dan itu yang membuatnya berguna sama sekali: artikel yang terbit hari ini masuk hari ini, tanpa deploy. Gagal dengan sopan — API mati menghasilkan sitemap berisi beranda saja, bukan 500, karena sitemap yang error mengajari perayap berhenti memintanya | 2026-09-04 |
| D76 | `canonical` dipasang untuk SETIAP rute, termasuk yang tidak dikenal CMS | Ia bukan pernyataan editorial melainkan teknis: "alamat resmi halaman ini yang ini". Tanpanya `/news`, `/news/`, dan `/news?category=Tournament` terbaca sebagai tiga halaman berisi hal yang sama. Memakai `route.path`, bukan `fullPath` — query string justru yang harus dibuang | 2026-09-04 |
| D77 | Kartu publikasi memakai `UiDocumentCard`, sampul dokumen dihapus | Desain berubah: `168:8582` — node yang dikutip implementasi lama — kini menggambar kartu 560 ringkas yang sama persis dengan rak press release, dan revisi terbarunya (`1010:2742`) sama. Tile 560×488 bersampul sudah tidak ada di file. Ini **mencabut kalimat terakhir D51**: `coverImageUrl` ada semata karena publikasi satu-satunya rak yang menggambar sampul, dan sekarang tidak ada rak yang menggambarnya — field-nya, `decor-publication-union.svg`, dan berkas sampulnya ikut dihapus. `documents.cover_image_path` di kontrak API akar jadi tak terpakai; membuangnya keputusan dua pihak, jadi dibiarkan sampai disepakati | 2026-09-08 |
| D78 | Rak dokumen diminta per **section**, bukan per kategori | Dua rak bisa menarik kategori yang sama — Statutes & Constitution dan Governance Repository dua-duanya `Governance Documents` — dan selama permintaannya per kategori keduanya mencetak dokumen yang sama persis, tanpa ada seorang pun yang bisa memutuskan sebaliknya. `getSectionResources("governance.repository")` menyerahkan pilihan itu ke backoffice (layar "Documents per Halaman"). Rak yang belum dikurasi dijawab "N terbaru dari kategorinya", jadi tidak ada halaman yang kosong menunggu admin. Kunci yang tidak dikenal ditolak 422, bukan `[]`: tiap rak menyembunyikan diri saat kosong, jadi salah ketik yang dijawab array kosong tidak terlihat di satu layar pun. `?category=` tetap dipakai dua tempat yang memang bukan rak — arsip press (seluruh kategori, tanpa batas) dan dua tombol regulasi `/domino` (sisa kategori di luar rak) | 2026-09-09 |
| D79 | Kosakata kategori dipendekkan jadi enam, dan dua dihapus | Permintaan pemilik repo. `Reports & Publications` → `Publication`, `Media & Press Releases` → `Press Releases`. `Integrity & Ethics` dan `Membership Documents` dibuang: halaman Integrity dan Members tidak pernah menggambar rak dokumen, jadi keduanya cuma menjanjikan tempat tayang yang tidak ada — persis yang dulu ditandai `planned` di `config/dwf.php` supaya layar Documents tidak berbohong. Backoffice memindahkan barisnya di perubahan yang sama; mock di repo ini ikut diperbaiki, dan itu menutup bug terpisah — sejak rename 2026-09-05 nilai `category` di mock masih kosakata lama, jadi SELURUH rak dokumen kosong di mode mock tanpa satu pun galat | 2026-09-09 |
