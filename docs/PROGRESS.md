...
**Struktur repo (2026-08-26):** akar repo memuat dua project terpisah —
`landing-page/` (seluruh isi akar lama, termasuk `docs/` ini) dan `backoffice/`
(kerangka folder kosong). Akar tanpa `package.json`; perintah dijalankan dari
dalam folder project (D70).
...
Cakupan di PRD §5. Portal sendiri belum dimulai; sejak 2026-08-26 ia punya
tempat — `backoffice/`, project terpisah dengan kerangka folder saja (D70).
Yang berjalan dua belas
...
| 2026-08-24 | Layar loading ditambahkan di akar `app/` — animasi CSS, bukan `motion`: fallback justru tampil saat JS belum punya apa-apa, jadi animasi yang menunggu hidrasi akan diam persis ketika dibutuhkan. Terverifikasi lewat jeda buatan 1,5 detik, lalu jedanya dicabut |
| 2026-08-24 | Tab filter (side tab + pil arsip berita) prefetch tujuan penuh, bukan cuma cangkang bersama — rute dinamis membuat default Next menyisakan render server di tiap klik. Tab FAQ juga berhenti melompat ke atas |
| 2026-08-24 | Bar progres navigasi ditambahkan di puncak layar — menyala saat item navbar diklik, padam setelah halaman baru terpasang (`usePathname` di root layout), dengan minimum tampil 450ms dan pengaman 8s |
...
| 2026-08-26 | Repo dipecah jadi dua project: seluruh isi akar lama pindah utuh ke `landing-page/` — `src/`, `public/`, config, `docs/`, `deploy/`, `README.md`, `CLAUDE.md`, `.gitignore` — dan `backoffice/` dibuat sebagai kerangka folder kosong. Atas permintaan pengguna: "beda project intinya" (D70). Bukan workspace Bun, bukan route group |
| 2026-08-26 | Pemindahan dibuat **rename murni** atas permintaan pengguna ("supaya git nya ga berubah"). Percobaan pertama memecah `.gitignore` dan menulis ulang `README.md` + `CLAUDE.md` di akar; itu dibatalkan — karena `docs/` ikut pindah, path relatif di dalam dokumen (`src/`, `public/assets/`, `lib/api/client.ts`, pohon folder RULES §2) tetap benar tanpa disunting sama sekali |
| 2026-08-26 | Akar repo kini hanya `landing-page/`, `backoffice/`, dan `CLAUDE.md` pendek yang menunjuk ke `landing-page/CLAUDE.md` — supaya sesi yang dibuka di akar tidak kehilangan aturan Bun dan ejaan brand. **Tanpa `.gitignore` di akar** (permintaan pengguna): dua folder project punya sendiri, dan `.DS_Store` akar ditangani `.git/info/exclude` — lokal ke klon, jadi tidak menambah file ke repo. Konsekuensinya klon lain perlu melakukan hal yang sama, atau menaruh `.DS_Store` di gitignore global |
| 2026-08-26 | Diverifikasi setelah pemindahan: `bunx next build` dari `landing-page/` lolos, 16 halaman, 14 rute — sama seperti sebelum pindah; `bunx tsc --noEmit` dan `bunx eslint` bersih. `.next/` lama dihapus, jadi `LayoutProps` (tipe generated Next) sempat hilang sampai build meregenerasinya || 2026-09-01 | Kartu S3 (upcoming match) digambar ulang dari node `24:1025`. Redraw-nya datang dua kali dan isinya sama: file REVISION (`VHKnrwJJW8Se8g1IUyELsY`) lalu file acuan PRD §5 (`oY2v2wq359rIRK4KaItmxc`) — seluruh node id, posisi dan dimensi kartu **dan hero di sekitarnya** identik di keduanya, jadi angkanya dikutip dari file acuan. Hasilnya: 498 × 292, bendera pindah ke samping judul, padding 24 → 20, jarak 24 → 16, garis pemisah dicabut dan timer masuk kotak gelap `rgba(14,14,14,.5)` radius 12. Sudut bawah kini siku (`24 24 0 0`) — isian dan stroke-nya sudah nol alpha sebelum kaki kartu |
| 2026-09-01 | Judul kartu jadi nama acara (data), bukan label tetap "Upcoming Match" — desain revisi menaruh nama acara di slot itu. Labelnya tetap dipakai sebagai awalan yang hanya dibacakan, supaya landmark-nya tidak cuma bernama "World Championship 2026" |
| 2026-09-01 | **Overhang S3 ke S4 dicabut** (dilaporkan pengguna). `-mt-[13.3%]` menaruh *puncak* kartu di `y` desain, jadi angkanya hanya benar selama tinggi kartunya tidak berubah — terhadap kartu 292px pun masih menyisakan 37px di bawah hero pada 1920. Diganti: pembungkusnya `h-0` + `items-end`, jadi kaki kartu selalu mendarat di tepi bawah hero berapa pun tingginya, lalu `-translate-y-5` mengangkatnya 20px sesuai desain. S4 kini mulai persis di tepi bawah hero |
| 2026-09-01 | Token `--gradient-divider` dihapus — satu-satunya pemakainya adalah garis pemisah S3 yang sudah tidak ada di desain |
| 2026-09-01 | Kartu masih 314px terhadap desain 292px setelah redraw — selisihnya satu properti: Figma tidak memberi line height pada digit Bebas 64px, dan auto untuk font itu setinggi capsnya (kotak `24:1029` 99px = 12 + 45 + 8 + 22 + 12). `leading-none` bukan hal yang sama — 1.0 × 64 = 64px — jadi digitnya kini `leading-[0.7]`, rasio Figma sendiri. Tinggi kartu jadi 295 |
| 2026-09-01 | Tinggi baris header & kotak timer **diukur dari ekspor PNG**, bukan dijumlah: keduanya hug-height, jadi Figma tidak melaporkan angkanya. Frame ber-fill (kartu, kotak timer) mengekspor kotak sebenarnya; frame tanpa fill mengekspor ink-nya, jadi angka baris header diturunkan dari sisa 292 dan bukan dari ekspornya |
| 2026-09-01 | Leading rapat **tidak** dipakai pada headline hero meski rasionya sama (`2:4`: Bebas 156 di kotak 109): headline itu gradien di balik `bg-clip-text`, dan background hanya dicat di dalam kotak — mengetatkan leading-nya akan memangkas caps hurufnya |
| 2026-09-01 | **Hero home diukur ulang ke frame 1080, bukan 1040.** Hero disliced dari file yang menggambarnya 1040 tinggi; file acuan menggambarnya 1080, dan semua isinya ditempatkan sebagai pecahan dari frame itu. `54.17vw` → `56.25vw`, lalu tiap persentase `lg:` dibagi ulang dengan 1080: batu bawah 62.5% → 63.9%, ubin 16.9%/61.2% → 16.3%/58.9%, vignette kaki 36% → 34.3%, tagline 36% → 35.3%, headline 44% → 42.6%. Ini juga yang membuat kartu S3 mendarat 37px lebih tinggi dari desain (dilaporkan pengguna) — kartunya menggantung di kaki hero, jadi ia ikut turun begitu framenya benar |
| 2026-09-01 | Ubin domino hero berhenti di `scale: 0.8` dan −6° — Figma tidak memberi rotasi maupun skala apa pun pada `24:933`, jadi istirahatnya seharusnya `scale: 1` di `rotate: 0`. Skala 0.8 mengecilkan layer ke pusatnya sendiri, jadi kaki ubinnya ~64px di atas posisi desain: itu keluhan "animasinya kurang ke bawah full". Entrance-nya tetap datang dari sedikit lebih kecil dan lebih miring |
| 2026-09-01 | **Gradien headline & tagline hero tidak pernah sampai putihnya.** Keduanya `bg-clip-text`, dan background dicat selebar kotak ELEMEN sebelum dipotong ke glif — `inset-x-0` membuat kotak itu selebar 1920, jadi ujung putih gradiennya jatuh di margin dan tipenya cuma kebagian bagian tengah yang abu. Figma mengecat gradien selebar node teksnya (479px / 1385px). Diperbaiki dengan `lg:w-fit lg:mx-auto lg:px-0` — kotaknya memeluk tipe, `inset-x-0` tetap yang memusatkan |
| 2026-09-01 | Leading headline sengaja **tidak** ikut dirapatkan seperti digit countdown: kotak 109px di bawah caps 114px akan memangkas hurufnya, karena background hanya dicat di dalam kotak. Dari `lg` headline diposisikan absolut, jadi leading berlebihnya tidak memakan tata letak apa pun |
| 2026-09-01 | **Navbar diskalakan proporsional, bukan dikecilkan.** Diukur: pil menu 1278px baik di Figma maupun di build — ia tidak lebih besar dalam piksel, tapi 1283px itu 67% dari jendela 1920 dan 71% dari jendela 1800, dan empat poin itulah yang terbaca sebagai bar yang berat. Tipe (`--text-nav`), padding item dan tinggi logo kini memakai slope desainnya sendiri (18/1920, 20/1920, 12/1920, 60/1920) dengan lantai = langkah 15px lama, jadi barisnya menahan 67% di tiap lebar. Langkah `menu-lg` pada menu dicabut karena slope-nya sudah menggantikan |
