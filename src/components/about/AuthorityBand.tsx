 ...
 *
 * `priority`: it sits immediately below the fold and is the largest image on
 * the page, so it is what LCP lands on for anyone who scrolls at all.
 */
export function AuthorityBand() {
  return (
    <div className="relative h-[56vw] overflow-hidden lg:h-[44.27vw]">
      {/* Slow — 8% of its own height, the same rate S4's building trails the
          page at. A backdrop that keeps pace with the copy stops reading as
          being behind it.

          The layer is taller than the frame so there is something to travel
          into: at 8% the picture would otherwise pull its own bottom edge up
          into view. */}
      <ParallaxLayer speed={8} className="absolute -inset-y-[6%] inset-x-0">
        <Image
