    ...
    <section aria-labelledby="details-heading" className="flex flex-col gap-8">
      ...
      <dl className="flex flex-col gap-8">
        <div className="flex flex-col gap-3">
          <dt className="font-sans text-label-muted text-[length:var(--text-eyebrow)] leading-7 font-medium">
          ...
          <dd className="font-sans text-[length:var(--text-body-sm)] leading-8 text-white">
            {CONTACT_COPY.address.map((line) => (
        ...
        <div className="flex flex-col gap-3">
          ...
          <dd className="font-sans text-[length:var(--text-body-sm)] leading-8 text-white">
            {/* A real `mailto:`, unlike the footer's. That address has no
                ...
                somewhere that cannot receive mail is worse than none; this one
                is well formed, which is exactly the condition the footer's own
                note names for when its address becomes a link. */}
            <a
              href={`mailto:${CONTACT_COPY.email}`}
              className="focus-visible:ring-gold underline decoration-from-font underline-offset-4 transition-colors hover:text-white/70 focus-visible:ring-2 focus-visible:outline-none"
            >
              {CONTACT_COPY.email}
            </a>
          </dd>
              ...
              {FOOTER_SOCIALS.map((social) => (
                <li key={social.id}>
                  <a
                    ...
                    className="focus-visible:ring-gold flex size-12 items-center justify-center rounded-[var(--radius-btn)] bg-white/10 transition-colors hover:bg-white/20 focus-visible:ring-2 focus-visible:outline-none"
                  >
