 ...
 * tile, which sets its copy ON the picture behind a scrim; here the two sit
 * side by side and the text is on the page background, so there is no wash and
 * no clamp — the headline has a whole column to wrap in.
 *
 ...
 *
 * The whole card is the link. The badge is a 48px target beside a 250px
 * picture, so it stays as the visual cue (`aria-hidden`, not focusable) and the
 * anchor is stretched over the card behind it — one tab stop per story, with
 * the headline as its accessible name.
 */
export function DevelopmentNewsCard({ article }: { article: NewsArticle }) {
  return (
    <article className="group relative flex items-center gap-4 lg:gap-6">
      {/* 250 × 250 in Figma. `shrink-0` because the headline beside it is the
          part that should give way: without it a long title squeezes the
          square into a rectangle and the crop moves. Sized in `vw` above `lg`
          so the pair keeps its proportions, with a floor that keeps the
          thumbnail recognisable on a phone. */}
      <div className="relative size-24 shrink-0 overflow-hidden rounded-[var(--radius-glass)] sm:size-32 lg:size-[max(8rem,13.02vw)]">
        <Image
          src={article.thumbnailUrl}
          // Empty: the headline beside it is the card's whole content, and
          // naming the picture would announce the story twice. Same call S8's
          // tiles make.
          alt=""
          fill
          sizes="(min-width: 1024px) 13vw, 128px"
      ...
      <div className="flex flex-col gap-2 lg:gap-3">
        ...
        {/* Inter SemiBold 36/44. Figma writes two of these four headlines with
            a hard newline and sets the style to `textCase: TITLE`; neither is
            reproduced — the break falls where the column puts it, and the case
            is the string's own (D40). */}
        <h3 className="font-sans text-[length:var(--text-body-lg)] leading-[1.22] font-semibold text-white">
          ...
          <a
            ...
            aria-label={DEVELOPMENT_NEWS_COPY.readLabel.replace(
              "%s",
              article.title,
            )}
