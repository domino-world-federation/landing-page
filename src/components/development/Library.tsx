 ...
 * so the line is a prop. The date is formatted here rather than stored
 * formatted — see `formatLongDate` for why the time zone is pinned.
 *
 * Server Component (`async`), like `Resources` and `Regulations`.
 */
export async function Library() {
  const documents = await getResources("Development")
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-10 lg:flex-row lg:gap-[13.96vw]">
        <div className="flex flex-col gap-6 lg:shrink-0 lg:grow-[356] lg:basis-0 lg:gap-9">
          <Reveal y={32}>
        ...
        <div className="lg:grow-[1136] lg:basis-0">
          {/* Figma wraps four fixed 560px cards inside 1136, which is a 2×2
              stated as a wrapping row. Written as a real grid, and 2-up only
              from `menu-lg` for the reason S10 measured: the pill inside each
              card is a fixed 160px, so what governs is how much of the card is
              left for the title, and below 1600 a two-column row takes these
              titles to three lines against the design's one. */}
          <div className="grid auto-rows-fr gap-4 menu-lg:grid-cols-2">
            {documents.map((doc, i) => (
              ...
              >
                <ResourceCard
                  doc={doc}
                  // `publishedAt` is optional on the type — most documents in
                  // the library have none, and only this section prints one.
                  // Falling back to the category rather than to an empty line
                  // keeps the card's shape if a document arrives undated.
                  meta={
                    doc.publishedAt
                      ? formatLongDate(doc.publishedAt)
                      : doc.category
                  }
                  downloadLabel={LIBRARY_COPY.downloadLabel}
                />
              </Reveal>
            ))}
          </div>
