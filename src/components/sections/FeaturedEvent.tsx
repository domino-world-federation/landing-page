 ...
 * so the card sits between the name it belongs to and the description of it.
 * All three belong to `EventShowcase`, which owns the pager — they change
 ...
 *
 * The fetch and the band itself stay on the server; only the columns are client
 * (RULES §5).
 */
export async function FeaturedEvent() {
  ...
  return (
    <section
      ...
      className="relative w-full bg-white px-5 py-16 text-black md:px-10 lg:px-20 lg:py-[3.125vw]"
    >
      <h2 id="featured-event-heading" className="sr-only">
        {FEATURED_EVENT_COPY.heading}
      </h2>

      {/* The eyebrow is set against the section's left edge in Figma, outside
          the three columns rather than above them — it labels the whole band.
          Below `lg` there is no room beside the content, so it sits over it. */}
      <div className="flex flex-col gap-8 lg:flex-row lg:gap-11">
        <p
          aria-hidden="true"
          className="font-sans text-xl leading-7 font-medium whitespace-pre-line text-black"
        >
