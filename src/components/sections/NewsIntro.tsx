...
/**
 * How far the photograph rides up as the section arrives, as a percentage of
 * the section's own height — negative, so it moves AGAINST the scroll and the
 * desk lifts while the page comes down past it.
 *
 ...
 * middle outwards, and at the design's 64px a deeper start means the outer
 * words travel far enough to read as a zoom. Scaling text is a transform on one
 ...
 * (`53:3070`), and the export is that stack already flattened and cropped to
 ...
 * background behind it — and that strip is the colour the image's own last rows
 * already are, so the seam is invisible. Moving it the other way would pull the
 * bright, detailed top edge down into the frame and expose a hard cut.
 *
 * Two moving layers, well under the 3–4 ceiling of RULES §12, and both move on
 * `transform`/`opacity` only. Server Component; only the two wrappers are
 * client.
 */
export function NewsIntro() {
  return (
    // The image's own 960/1920 ratio from `lg`, so every percentage below is a
    // Figma number as written. A floor for the same reason the hero has one:
    // the sentence does not shrink with the window, and pure ratio would close
    // the section around it on a narrow desktop.
    <section className="relative isolate flex min-h-[420px] flex-col items-center justify-center overflow-hidden px-5 py-24 md:px-10 lg:h-[max(560px,50vw)] lg:min-h-0 lg:justify-start lg:py-0 lg:pt-[17.86%]">
      ...
      >
        {/* `sizes="100vw"`: the picture always spans the viewport, so the
            browser can pick a candidate from the width alone. */}
        <Image
      ...
      {/* Node `55:3224`: 1008px wide and centred on the frame, its top at
          `y:343` — which is what the `pt-[17.86%]` on the section reproduces,
          343 being 17.86% of the design's 1920 width and the height being
          pinned to that width from `lg`. Below `lg` the section has no fixed
          ratio to measure against, so the sentence simply centres.

          The sentence comes in from blur, GROWS into its resting size and
          brightens as it settles — that is the section's whole entrance. It
          does not rise, though: the picture behind it is already travelling
          upwards, and a second thing moving in the same direction in a frame
          this empty reads as the section sliding rather than as the words
          arriving. Growing from the centre reads as approach instead, which is
          a different axis from the desk's drift and so keeps the two apart.
      ...
      <Reveal
        ...
        className="w-full"
      >
        <p className="font-sans mx-auto max-w-[1008px] text-center text-[length:var(--text-news-intro)] leading-[0.97] font-medium text-balance text-white">
