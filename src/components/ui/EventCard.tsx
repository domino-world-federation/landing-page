 ...
 * Nothing else on the card moves — the type, the panel and the light streaks
 * are fixed, which is what keeps the drift legible as depth rather than wobble.
 *
 ...
 * `whileInView` hands control back to `animate` when it leaves, which parks
 * both layers at their resting values instead of leaving a timer burning
 * against an off-screen element.
 *
 * Only `transform` is animated, and `prefers-reduced-motion` stops the loop
 * entirely rather than slowing it (RULES §12) — through the transition, never
 * by branching the markup, which is what would break hydration.
 *
 * The layers are stacked in Figma's own child order: gradient panel, watermark,
 * photograph, wordmark, light streaks.
 *
 * This is a Client Component because of those two animations. Everything around
 * it — the copy column, the buttons — stays on the server (RULES §5).
 */
export function EventCard({ event, className }: EventCardProps) {
  const prefersReducedMotion = useReducedMotion()

  // Reduced motion collapses the TRANSITION and leaves the tree alone. Both
  // sides render the same `initial`, and a zero-length tween lands the layers
  // at rest before the first paint.
  //
  ...
  // Where both layers sit when the card is off screen — their design values, so
  // a card that has never been looked at is the still design exactly.
  ...
  return (
    <div
      className={cn(
        // 520 × 720 in Figma. The ratio is carried rather than the pixels, so
        // the card can shrink on a narrow screen without the artwork inside it
        // re-composing — every layer below is placed as a percentage of this
        // box.
    ...
    >
      {/* The federation mark ghosted into the panel — node `52:3042`.
          Decorative: the emblem is already spelled out by the logo in the
          navbar, so naming it here would only repeat it (RULES §7).

          The export already carries Figma's 8% IN ITS ALPHA CHANNEL — the PNG's
          peak alpha is 20/255 ≈ 0.078, measured, not assumed — so an
          `opacity-[0.08]` class on top of it multiplied out to ~0.6% and the
          mark disappeared, which is what it was doing. The layer stays fully
          opaque and lets the asset's own alpha do the work; no filter, so the
          mark keeps its own pale tone against the gold. */}
      <motion.div
        aria-hidden="true"
        className="absolute top-[14.9%] left-[16.3%] h-[55%] w-[70.2%]"
        initial={{ scale: 1 }}
        animate={{ scale: 1, transition: rest }}
        whileInView={{ scale: WATERMARK_MAX, transition: drift }}
        viewport={viewport}
      ...
      {/* The trophy — node `52:3046`. Figma stretches a 1792 × 2400 photograph
          ...
          (`cropTransform` x-scale 0.747, offset 0.126), which is what centres
          the trophy in the frame. `object-cover` on a box of the slot's own
          ratio reproduces that: the height fills and the sides are what get
          trimmed. */}
      <motion.div
        className="absolute inset-y-0 left-[11.3%] w-[77.3%]"
        initial={{ y: "0%" }}
        animate={{ y: "0%", transition: rest }}
        whileInView={{ y: `${TROPHY_RISE}%`, transition: drift }}
        viewport={viewport}
        style={{ willChange: "transform" }}
      >
        <Image
      ...
      <span
        aria-hidden="true"
        className="font-display absolute bottom-[7.5%] left-0 w-full text-center text-[length:clamp(3rem,8.5vw,10.25rem)] leading-none tracking-[-0.0488em] text-white"
      >
        {FEATURED_EVENT_COPY.watermark}
        {eventYear(event.dateLabel)}
      </span>
      ...
      {/* eslint-disable-next-line @next/next/no-img-element -- an SVG served
          unoptimized either way (RULES §7), positioned by percentages against
          the card. next/image would need `fill` inside a positioned wrapper,
          and the wrapper would then have to carry the -20%/-16.3%/166.5%
      ...
      <img
        ...
        width={946}
        height={756}
        className="pointer-events-none absolute top-[-20%] left-[-16.3%] w-[166.5%] max-w-none mix-blend-plus-lighter"
      />
    </div>
  )
}
 ...
 */
function eventYear(dateLabel: string): string {
  return dateLabel.match(/\d{4}/g)?.at(-1) ?? ""
}
