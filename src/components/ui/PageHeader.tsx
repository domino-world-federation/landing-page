 ...
 *
 * Figma gives every one of these titles `filter: blur(7.5px)`. That is the
 * state it clears FROM, never an animated `filter` (RULES §11) —
 * `SharpeningHeadline` cross-fades two static-blur copies.
 *
...
export function PageHeader({
  ...
  return (
    ...
    <section className="flex min-h-[420px] flex-col justify-end gap-8 px-5 pt-32 pb-14 md:px-10 lg:min-h-[27.6vw] lg:gap-16 lg:px-20 lg:pb-[5.2vw]">
      {back && (
        <a
          href={back.href}
          className="font-display focus-visible:ring-gold group flex w-fit items-center gap-3 text-[length:var(--text-display-caption)] leading-none text-[#aaaaaa] transition-colors hover:text-white focus-visible:ring-2 focus-visible:outline-none"
        >
          {/* eslint-disable-next-line @next/next/no-img-element -- a 32px
              inline SVG sized in CSS. The shared asset already points LEFT,
          ...
          <img
            ...
            width={32}
            height={32}
            className="size-8 invert transition-transform duration-200 group-hover:-translate-x-0.5"
          />
          {back.label}
        </a>
      )}
      ...
      <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between lg:gap-16">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:gap-12">
          <h1 className="font-sans text-[length:var(--text-page-title)] leading-[1.1] font-medium text-white">
            <SharpeningHeadline lines={title} />
          </h1>
          {meta}
        </div>
